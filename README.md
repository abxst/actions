...
- Findings land in **GitHub Code Scanning** as SARIF — permanent, deduplicated, annotated onto PR diffs, auto-closed when fixed
...
### 2.3. Enable Code Scanning

Findings are uploaded as SARIF to GitHub Code Scanning, which is where they live permanently.

- **Public repos**: available for free, nothing to enable.
- **Private repos**: requires GitHub Advanced Security. Without it the upload step fails — harmlessly, because it is marked `continue-on-error`, but it will show a red ✗ on an otherwise green job. Set `sarif-upload: 'false'` to skip it entirely.

Findings then appear under **Security → Code scanning**, split by tool via the `semgrep` and `trivy` categories.
...
Push a commit to a test branch, open a PR into `main`, then check the **Actions** tab. If a gate fails, the step log prints a compact summary of exactly what blocked it; the full detail is in **Security → Code scanning**.
...
| `sarif-upload` | no | `'true'` | Upload SARIF to Code Scanning. Set to `'false'` on private repos without Advanced Security. |
...
| `sast-sarif` | Path to the full Semgrep SARIF report |
| `sast-gate-report` | Path to the Semgrep JSON report containing only blocking findings |
| `image-sarif` | Path to the full Trivy SARIF report |
| `image-gate-report` | Path to the Trivy JSON report containing only blocking vulnerabilities |

The four report files live in `$RUNNER_TEMP`, **not** in your workspace — a stray file in the build context would bust the Docker layer cache and, with a `COPY . .` Dockerfile, bake your vulnerability report into the shipped image.

Because they're in `$RUNNER_TEMP`, they are **deleted when the job ends**. Consume them in the same job, or rely on Code Scanning for the durable copy.
...
```
Semgrep (full)  → SARIF  → upload to Code Scanning
Semgrep (gate)  → JSON   → ✋ fails here if source findings hit the threshold
Build (local for scan)
Trivy (full)    → SARIF  → upload to Code Scanning
Trivy (gate)    → JSON   → ✋ fails here if CVEs hit the threshold
Push
```
...
- name: Comment blocking CVEs on the PR
  ...
  env:
    GATE: ${{ steps.docker.outputs.image-gate-report }}
...
### `Resource not accessible by integration` on the SARIF upload

**Cause**: the job is missing `security-events: write`, or the run is a PR from a fork (GitHub does not grant that scope to fork PRs).

**Fix**: add the permission. For fork PRs there is no fix — the upload step is `continue-on-error`, so it degrades to a red ✗ on that step without failing the build. Findings are still enforced by the gate; only the Code Scanning upload is skipped.

### `Advanced Security must be enabled for this repository`

**Cause**: private repo without GitHub Advanced Security. Code Scanning is free for public repos only.

**Fix**: set `sarif-upload: 'false'`. The gates keep working; you lose the permanent record and the PR annotations.
...
A: The permanent copy is in GitHub Code Scanning (SARIF, categories `semgrep` and `trivy`). The JSON gate reports are transient files in `$RUNNER_TEMP`, exposed via outputs for same-job processing and deleted when the job ends.
...
2. **Reports are no longer printed as tables in the log.** Anything parsing the job log for Trivy output will break. Read the SARIF in Code Scanning, or the JSON gate reports via the new outputs.

To adopt gradually, start at `with-scan` (gates on `ERROR` only) and tighten from there.

Add `security-events: write` to your job's `permissions`, or set `sarif-upload: 'false'` if you're on a private repo without Advanced Security.
