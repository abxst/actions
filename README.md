...
Mục đích: chuẩn hoá pipeline Docker build/push cho các project cá nhân. Thay vì copy-paste 70 dòng YAML giữa các repo, mỗi project chỉ cần 1 step `uses:`.
...
Ví dụ phân loại:
- Frontend app, API backend, service deploy lên production → **with-scan**
- Internal tooling, dev playground, sandbox → **no-scan**
...
- **Push lên `main`** → build + scan + push lên `ghcr.io/abxst/<repo-name>`
...
| `image-name` | no | `${{ github.repository }}` | Image name không kèm registry (vd `abxst/my-app`) |
...
| `image-name-lc` | Image name dạng lowercase (`abxst/my-app`) |
...
- name: Update manifest repo for ArgoCD
  ...
  run: |
    ...
    yq -i ".spec.template.spec.containers[0].image = \"ghcr.io/abxst/my-app@${DIGEST}\"" deployment.yaml
...
**Nguyên nhân**: GHCR yêu cầu image name lowercase nhưng `github.repository` có thể chứa chữ hoa (vd `MyOrg/MyApp`).
...
A: Có. Đổi `registry` input (vd `docker.io`, `harbor.example.com`) và truyền đúng `registry-username` / `registry-password`. Token GHCR mặc định là `${{ secrets.GITHUB_TOKEN }}`, registry khác cần secret riêng.
...
|---|---|---|
| `@v1` | Default cho production | Auto nhận patch + minor, có thể có behaviour thay đổi nhẹ |
...
Khuyến nghị: pin `@v1` cho mọi caller bình thường để tự nhận patch.
